...
namespace TransCanada.Controllers
{
    ...
    public class Asp_AccountsController : Controller
    {
        ...
        public ActionResult InsertAccounts(AspNetAccountsModel Acc, HttpPostedFileBase file)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Acc.add_email))
                    sqlCommand.Parameters.AddWithValue("@add_email", Acc.add_email.Trim());
                else
                ...
                command.Parameters.AddWithValue("@address_Type", "HO");
                connection.Open();
               int Location_Id=Convert.ToInt32(command.ExecuteScalar());
                connection.Close();
                ...
                SqlCommand command2 = new SqlCommand("proc_insert_client_contact", connection);
                command2.CommandType = CommandType.StoredProcedure;

                if (!string.IsNullOrEmpty(Acc.email))
                {
                    command2.Parameters.AddWithValue("@Email1", Acc.email);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Email1", string.Empty);
                }
                if (!string.IsNullOrEmpty(Acc.Phone))
                {
                    command2.Parameters.AddWithValue("@Phone1", Acc.Phone);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Phone1", string.Empty);
                }
                if (!string.IsNullOrEmpty(Icontact.Role))
                {
                    command2.Parameters.AddWithValue("@Role", Icontact.Role);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Role", string.Empty);
                }
                if (!string.IsNullOrEmpty(Icontact.Title))
                {
                    command2.Parameters.AddWithValue("@Title", Icontact.Title);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Title", string.Empty);
                }
                if (!string.IsNullOrEmpty(Acc.Notes))
                {
                    command2.Parameters.AddWithValue("@Notes", Acc.Notes);
                }
                else
                {
                ...
                if (!string.IsNullOrEmpty(Acc.FirstName))
                {
                    command2.Parameters.AddWithValue("@firstname", Acc.FirstName);
                }
                else
                {
                    command2.Parameters.AddWithValue("@firstname", string.Empty);
                }
                if (!string.IsNullOrEmpty(Acc.LastName))
                {
                    command2.Parameters.AddWithValue("@Lastname", Acc.LastName);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Lastname", string.Empty);
                }
                if (!string.IsNullOrEmpty(Acc.email))
                {
                    command2.Parameters.AddWithValue("@email", Acc.email);
                }
                else
                {
                    command2.Parameters.AddWithValue("@email", string.Empty);
                }



                if (Acc.Phone != null)
                    command2.Parameters.AddWithValue("@cell", Acc.Phone);
                else
                    command2.Parameters.AddWithValue("@cell", string.Empty);
                if (!string.IsNullOrEmpty(Icontact.cell))
                {
                    command2.Parameters.AddWithValue("@officephone", Icontact.cell);
                }
                else
                {
                    command2.Parameters.AddWithValue("@officephone", string.Empty);
                }
                if (!string.IsNullOrEmpty(Icontact.Third_Phone))
                {
                    command2.Parameters.AddWithValue("@Third_Phone", Icontact.Third_Phone);
                }
                else
                {
                    command2.Parameters.AddWithValue("@Third_Phone", string.Empty);
                }
                connection.Open();
        ...
        {
            SqlConnection connection = new SqlConnection(this.TransConnString);
            SqlCommand sqlCommand = new SqlCommand("delete_client_details", connection);
            sqlCommand.CommandType = CommandType.StoredProcedure;
        ...
        public ActionResult ChangeClient(int id)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                using (SqlConnection con = new SqlConnection(this.TransConnString))
                {
                    string query = "get_user_Accounts_by_id";
                ...
                var list_useraccount = list_useraccounts.Where(x => x.Account_id == id);
            ...
            //return View();
            return RedirectToAction("UpdateAccounts", "Asp_Accounts",new { id=id});

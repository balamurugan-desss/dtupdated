            ...
            if (sqlDataReader.HasRows)
            {
                while (sqlDataReader.Read())
                {
                    ...
                    if (!string.IsNullOrEmpty(sqlDataReader["AccountNumber"].ToString()))
                        subServices.AccountNumber = Convert.ToInt64(sqlDataReader["AccountNumber"]);
                    else
